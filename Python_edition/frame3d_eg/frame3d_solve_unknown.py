...
def frame3d_solve_unknown(k,unknown_u_index,f,u,nodes,dof):
    # 利用分块矩阵计算未知的变量，力和位移。
    answer = []
    kkk = k.copy()  # 此处变换需要用深拷贝。
    a = unknown_u_index
    n = a.shape[0]
    b = np.zeros((dof * nodes - n, 1))
    j = 0
    for i in range(dof * nodes):
        if not (i + 1 in a):
            b[j, 0] = i + 1
            j = j + 1
    ...
    # 分块后的矩阵。
    k1 = kk[0:n, 0: n]
    k2 = kk[0:n, n: dof * nodes]
    k3 = kk[n:dof * nodes, 0: n]
    k4 = kk[n:dof * nodes, n: dof * nodes]
    ...
    # F.U为总节点位移、力的列阵。
    F = np.zeros((dof * nodes, 1), dtype=float)
    U = np.zeros((dof * nodes, 1), dtype=float)
    index_f_unknown = 0
    index_u_known = 0
    index_f_known = 0
    index_u_unknown = 0
