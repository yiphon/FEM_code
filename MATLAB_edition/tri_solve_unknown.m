function y = tri_solve_unknown( k,unknown_u_index,f,u,nodes )
%TRI_SOLVE_UNKNOWN Summary of this function goes here
...
a=unknown_u_index;
n=size(a,1);
b=zeros(2*nodes-n,1);       %b为已知节点位移的序号列阵。
j=1;
for i=1:1:2*nodes
    if ismember(i,a)==0
        b(j)=i;
...
k1=kk(1:n,1:n);
k2=kk(1:n,n+1:2*nodes);
k3=kk(n+1:2*nodes,1:n);
k4=kk(n+1:2*nodes,n+1:2*nodes);
u_solved=k1\(f-k2*u);
